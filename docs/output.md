...
- [MultiQC execution report](#multiqc-execution-report) - Auditable summary of inputs, outputs, checksums, and the execution environment
...
- `multiqc/`
  - `multiqc_report.html`: Standalone execution report suitable for display and archival.
  - `multiqc_data/`: Structured data parsed by MultiQC and the source data used to build the report.

</details>

The report is generated with the nf-core `multiqc` module after the Quarto report completes. It contains MD5 checksums for every samplesheet input and the rendered Quarto HTML report, plus the pipeline version, launch command, container identifiers, and available Python and R versions. Missing Python or R installations are reported as unavailable instead of failing the workflow. `tower.yml` exposes the HTML file to Seqera Platform as a pipeline report.
